...
if(isset($_REQUEST['submit'])){
   ...
   $password =$_POST['password'];
    ...
    $authentication_pwd=md5(sha1($password));
      ...
      $insert_data = "INSERT INTO driver_login (numbers, password) 
                  VALUES ('$user_number','$authentication_pwd')";


  if (mysqli_query($database_connection, $insert_data)) {
   
    $read_number_and_password="SELECT * FROM driver_registration WHERE phone_number='$user_number' AND passwords='$authentication_pwd' ";

    $run= mysqli_query($database_connection, $read_number_and_password);
    $count=mysqli_num_rows($run);
    if($run==true){
      if($count>0){
           setcookie("current_driver",$user_number,time()+(86400*7));
...
<form action ="#"  method="POST">
  ...
  <a href="pass_recovery">Forgat Password?</a>
  <p></p>
